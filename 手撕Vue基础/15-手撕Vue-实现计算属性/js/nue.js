// computed的实现其实就是先把computed的get方法挂载到$data中，然后在第一次编译模板的时候，调用到这个get方法，在内部会访问到它具体用的属性
// 比如name，然后这时候getName这个计算属性的更新就添加到了它关联的属性name的dep中，所以name一遍，getName也会重新渲染
    ...
    content: function (node, value, vm) {
        ...
        let val = value.replace(reg, (...args)=>{
            ...
            new Watcher(vm, args[1], (newValue, oldValue)=>{
                ...
                node.textContent = this.getContent(vm, value);  // 因为模板并不是在只有一个{{}}存在，所以每次都要重写渲染出内部数据
            });
...
class Watcher {
    // vm是vue示例，attr是观察的参数名，可能是一个深度的参数，比如time.h，cb就是该观察者的回调函数，在update的时候触发
    constructor(vm, attr, cb){
        this.vm = vm;
        this.attr = attr;
        this.cb = cb;
        // 在创建观察者对象的时候就去获取当前的旧值（获取旧的值的主要目的是在第一次创建模板的时候，触发Observe添加的get，将该watcher放到对应属性的订阅器中）
